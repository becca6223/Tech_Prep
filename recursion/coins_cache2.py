...
#memoization should be checked at the base case -> better, cleaner code 
def coins(n):
	dollars = [25, 10, 5, 1]
	cache = {}
...
def getAllCoinsCombo(remainDollar, dollars, cur_dollar, cache):
	if dollars[cur_dollar] == 1:
		...
		for i in range(int(remainDollar/dollars[cur_dollar]), -1, -1):
			...
			if remainDollar in cache and cache[remainDollar][uniqueID(dollars[cur_dollar])] != -1:
				ways += cache[remainDollar][uniqueID(dollars[cur_dollar])]
			else:
